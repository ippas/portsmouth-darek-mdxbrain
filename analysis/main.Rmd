...
cov3 <- c(rep(0, 20), rep(1, 20))  # At least one covariate is confounded with batch! Please remove confounded covariates and rerun ComBat-Seq
...
counts <-
  ...
  filter(rowMeans(across(contains('sample'))) > 1) %>%
   # filter out by fold
  pivot_longer(contains('sample'), names_to = 'sample_id', values_to = 'value') %>%
  left_join(rename(sample_info, sample_id = id), by = 'sample_id') %>%
  group_by(gene_stable_id, genotype, age_days, tissue) %>%
  mutate(m = mean(value)) %>%
  ...
  ungroup %>%
  filter(max_m - min_m > 1) %>%
  select(-mouse_id, -genotype, -age_days, -tissue, -m, -min_m, -max_m) %>%
...
anova_rds_path <- 'data/mdx_3way_anova_5fold_2fold_mtrrna.RDS'
