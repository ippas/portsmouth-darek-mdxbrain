...
anova_rds_path <- 'data/mdx_3way_anova_007.RDS'
...
mdx_anova_df <- as.data.frame(mdx_anova)
...
fold_th <- 0
ttest_genotype_bonf %>%
  left_join(mdx_anova %>% select(gene_stable_id, starts_with('fold_')), by = 'gene_stable_id') %>%
  filter(
...
bonf_th <- 0.01
fold_th <- 1
for (group in list(c(10, 'cerebrum'), c(10, 'cerebellum'), c(70, 'cerebrum'), c(70, 'cerebellum'))){
  post_hoc_genes <-
    ttest_genotype_bonf %>%
    filter(age_days == group[1], tissue == group[2]) %>%
    filter(p_bf < bonf_th) %>%
