...
anova_rds_path <- 'data/mdx_anova.RDS'
if (!file.exists(anova_rds_path)) {
  if (Sys.info()['sysname'] == 'Windows') {
    plan(multisession)
  } else {
    # run with: Rscript -e "
    #   rmarkdown::render('analysis/main.Rmd', knit_root_dir = xfun::proj_root())
    # "
    plan(multicore)
  }
  ...
  mdx_anova <- bind_rows(mdx_anova)
  mdx_anova <- bind_cols(
    counts %>% select(!contains('sample')),
    mdx_anova
  )

  saveRDS(mdx_anova, anova_rds_path)
}
```
