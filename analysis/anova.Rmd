...
compute_anova <- function(x, template) {
  template %>%
    ...
    # { summary(aov(
    #   value ~ genotype + age_days + tissue
    #   + genotype*age_days + genotype*tissue + age_days*tissue
    #   + genotype*age_days*tissue, data=.
    # )) } %>%
    # `[[`(1) %>%
    # as_tibble(rownames = 'fct') %>%
    { summary(aov(
      value ~ genotype + age_days + tissue
        + genotype*age_days + genotype*tissue + age_days*tissue
        + genotype*age_days*tissue + Error(mouse_id/tissue),
      data=.
      )) } %>%
    { bind_rows(
      .[['Error: mouse_id']][[1]] %>% as_tibble(rownames = 'fct'),
      .[['Error: mouse_id:tissue']][[1]] %>% as_tibble(rownames = 'fct')
    ) } %>%
    mutate(fct = trimws(fct)) %>%
    filter(fct != 'Residuals') %>%
    pivot_wider(id_cols = c(), names_from = fct, values_from = `Pr(>F)`) %>%
    select(any_of(c('genotype', 'age_days', 'tissue', 'genotype:age_days', 'genotype:tissue', 'age_days:tissue', 'genotype:age_days:tissue')))
}
```
