...
```{r fit_model}
edger_model <- function(x, sample_info_groups, tissue=NA, contrast='genotype', bcv=FALSE){
  mdx_groups. <-
    sample_info_groups %>%
    `[`(match(colnames(x), .$id), ) %>%
    pull(group) %>%
  ...
  dge_counts <- estimateDisp(dge_counts, design=design)

  # other BCV
  if (bcv) {
    bcv_tagwise <- list()
    plotBCV(dge_counts)
    bcv_tagwise[['all']] <- sqrt(dge_counts$tagwise.dispersion)
    bcv_common[['tissue']] <<- c(bcv_common[['tissue']], tissue)
    for (gr in c('wt_10', 'wt_70', 'mdx_10', 'mdx_70')){
      sample_info_sub <- filter(sample_info_groups, group == gr)
      y <- x[, sample_info_sub$id]
      mdx_groups_sub. <-
        sample_info_sub %>%
        `[`(match(colnames(y), .$id), ) %>%
        pull(group) %>%
        factor()
      dge_counts_sub <- DGEList(counts = y, group = mdx_groups_sub.)
      dge_counts_sub <- dge_counts_sub[keep_counts, , keep.lib.sizes = FALSE]
      dge_counts_sub <- calcNormFactors(dge_counts_sub)
      dge_counts_sub <- estimateDisp(dge_counts_sub)
      plotBCV(dge_counts_sub)
      bcv_tagwise[[gr]] <- sqrt(dge_counts_sub$tagwise.dispersion)

      bcv_common[[gr]] <<- c(
        bcv_common[[gr]],
        sqrt(dge_counts_sub$common.dispersion)
      )
    }
    bcv_common[['all']] <<- c(
      bcv_common[['all']],
      sqrt(dge_counts$common.dispersion)
    )
  }
  ...
  if (bcv) {
    out <- mutate(
        out,
        bcv = bcv_tagwise$all,
        bcv_wt_10 = bcv_tagwise$wt_10, bcv_wt_70 = bcv_tagwise$wt_70,
        bcv_mdx_10 = bcv_tagwise$mdx_10, bcv_mdx_70 = bcv_tagwise$mdx_70,
        .after = row_index
      )
  }
  out
...
.cerebrum_edger <- function(contrast, bcv = FALSE) {
  edger_model(
...
edger_genes_cerebrum <-
  ...
  rename_with(.fn = ~ paste0(.x, '.genotype'), .cols = -c(row_index, starts_with('bcv'))) %>%
  full_join(
    rename_with(
...
.cerebellum_edger <- function(contrast, bcv = FALSE) {
  edger_model(
...
as_tibble(bcv_common) %>% write_csv('results/BCV-common.csv')
as_tibble(bcv_common) %>%
  mutate(across(-tissue, ~ round(., 3)))
```
