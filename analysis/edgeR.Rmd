...
path <- paste0('results/genotype-edger-', tiss, '-0.1.png')
png(path, width = 1000*2.03, height = 2200*2.03, pointsize = 25)
par(cex.axis=1.3)
...
tibble(gene_name = names(ct_heatmap_order), cluster = ct_heatmap_order) %>%
  write_csv(paste0('results/edger-', tiss, '-clusters-0.1.csv'))
```
...
```{r draw_heatmap_03, echo=FALSE, fig.align="center", fig.height=14, fig.width=8, results='asis', include=TRUE, warning=FALSE}
fdr_th <- 0.1
