...
```{r draw_heatmap_03, echo=FALSE, fig.align="center", fig.height=20, fig.width=8, results='asis', include=TRUE, warning=FALSE}
fdr_th <- 0.1
...
path <- paste0('results/complex-heatmap/genotype-edger-', tiss, '_raster.svg')
svg(path, width = 10, height = 16)
h_cerebrum <- Heatmap(
  .x,
  ...
  top_annotation = c(
    columnAnnotation(
      ...
      col = list(
        'age' = c('10' = 'green', '70' = 'magenta'),
        'genotype' = c('wt' = 'yellow', 'mdx' = 'black')
      )
    )
  ),
  right_annotation = rowAnnotation(
    `genotype 10 days` = plot_data$genotype_10,
    `genotype 70 days` = plot_data$genotype_70,
    col = list(
      'genotype 10 days' = c('sig'='green', 'ns'='white'),
      'genotype 70 days' = c('sig'='magenta', 'ns'='white')
    )
  ),
  use_raster = TRUE,
)
...
ct <- cutree(as.hclust(row_dend(h_cerebrum)), k = 4)
ct_heatmap_order <- ct[row_order(h_cerebrum)]
...
path <- paste0('results/complex-heatmap/genotype-edger-', tiss, '.svg')
svg(path, width = 10, height = 16)
...
ct <- cutree(as.hclust(row_dend(h_cerebellum)), k = 4)
ct_heatmap_order <- ct[row_order(h_cerebellum)]
