...
svg(paste0(.path_base, '.svg'), width = 13, height = 20)
h_cerebrum <- Heatmap(
  .x,
...
svg(paste0(.path_base, '.svg'), width = 12, height = 6)
...
png(paste0(.path_base, '.png'), width = 1000, height = 600, res = 120)
h_cerebellum <- Heatmap(
  .x,
...
genes_wawa <- setdiff(genes_wawa, c('Dmd', 'Gphn', 'Dag1', 'Tfam', 'Pklr'))
...
```{r draw_heatmap_selected_cerebrum, echo=FALSE, fig.align="center", fig.height=20, fig.width=12, results='asis', include=TRUE, warning=FALSE}
plot_data <-
  counts_cerebrum %>%
  full_join(edger_genes_cerebrum, by = c('gene_stable_id', 'gene_name')) %>%
  filter(gene_name %in% genes_wawa) %>%
...
svg('results/selected-genes/selected-genes-cerebrum.svg', width = 11, height = 7.3)
h_cerebrum <- Heatmap(
  .x,
